...
def run_peptide_energy_landscape_calc( energy_fxn, config, targets, test_name, xml, pH_mode=False, pH_value=7 ): 
	"""
	...
	Arguments: 
		...
		pH_Mode = Use pH energy in landscape calculations
		pH_value = Physiological pH is 7
	"""

    print( "Making energy landscapes for the helical peptide set", targets ) 
    ...
    for case in test_cases:
        ...
        if ( pH_Mode == True ): 
  		...
  		casedir = outdir + "/" + case
 		if ( pH_mode == True ): 
 			casedir = casedir + "_pH" + str(pH_value)
