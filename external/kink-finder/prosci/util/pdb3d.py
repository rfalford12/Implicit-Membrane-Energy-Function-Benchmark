...
def rmsd_static(coordA, coordB, atom_types=("N", "CA", "C", "O")):
    "Calculate RMSD between two corresponding set of residues, without moving anything."
...
def gdt(coordA, coordB, distance_cutoffs, length=None):
    ...
    drange.reverse() # we want to start by checking the larget cut-offs, then move on to the smaller ones
...
def averageCoordinates(fragScaffold, fragPrediction, invertWeights=False):
    ...
    for i,rS in enumerate(resS):
      ...
      if not rS.has_atom_type('CA') or not rP.has_atom_type('CA'):
        raise IllegalState("Residue must contain a CA atom:\nrS:\n"+str(rS)+"rP:\n"+str(rP))
        
      rS, rP = intersectAtomTypes(rS, rP)
        #raise IllegalState("Residue must contain only/all main-chain atoms (and optionally a CB):\nrS:\n"+str(rS)+"rP:\n"+str(rP))
