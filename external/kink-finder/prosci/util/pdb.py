...
def diffPdb(bigModel, smallModel):
  ...
  for start, end in smallModel.xresidue_boundaries():
    ...
    excludeMe.add((atm.ires, atm.res, atm.chain))
  
  difference = Pdb("difference", [])
  for start, end in bigModel.xresidue_boundaries():
    atm = bigModel[start]
    if (atm.ires, atm.res, atm.chain) not in excludeMe:
...
def intersectPdb(models):
  ...
  for start, end in models[0].xresidue_boundaries():
    atm = models[0][start]
    intersection.add((atm.ires, atm.res, atm.chain))
  
  for m in models[1:]:
    res_set=set([])
    for start, end in m.xresidue_boundaries():
      atm = m[start]
      res_set.add((atm.ires, atm.res, atm.chain))
  ...
  for m in models:
    ...
    for start, end in m.xresidue_boundaries():
      atm = m[start]
      if (atm.ires, atm.res, atm.chain) in intersection:
...
class Atom:
  ...
  def __getitem__(self, i):
      if isinstance(i, int):
          return eval("self."+Atom.fields[i])
      else:
          return eval("self."+str(i))
  ...
  def __str__(self):
      if self.hetatm:
        ...
        datatype = "ATOM  "
      return "%s%5d %1s%-3s%1s%-3s %1s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f          %2s%-2s" % (datatype, self.iatom, self.atom[-4:-3], self.atom[-3:], self.altloc, self.res, self.chain, self.ires, self.inscode, self.x, self.y, self.z, self.occup, self.b, self.element, self.charge)
  def __repr__(self):
...
class Pdb:
    ...
    def __init__(self, pdb, data=None, keepBefore1=False, deleteInsertions=False):
        self.parent = None
        self.code   = ""
        self.data   = []
    ...
    def __repr__(self):
        ...
        return 'Pdb("%s", %s)' % (self.code, repr(self.data))
    ...
    def get_backbone_no_O(self, allowAlternatives=False):
        """get_backbone() : Returns a Pdb object containing all backbone (N, CA, C, O) atoms."""
        return self.get_atoms_by_type(("C", "N", "O"), allowAlternatives)
    ...
    #~ def filter_missing_CA(self):
        #~ """filter_missing_CA() : deletes residues that do not contain a CA atom
        #~ 
        #~ This is now run automatically in the Pdb constructor
        #~ """
        #~ slices = []
        #~ for start,end in self.xresidue_boundaries():
          #~ isok=False
          #~ for ix in xrange(start,end):
            ...
            #~ if atm.atom == "CA":
              ...
              #~ break
          #~ if isok:
            #~ if slices and slices[-1][1] == start:
              #~ slices[-1][1] = end
            #~ else:
              #~ slices.append([start, end])
        #~ 
        #~ if len(slices) == 1 and slices[0][0]==0 and slices[0][1] == len(self):
          #~ return False
        #~ 
        #~ newatoms = []
        #~ for start, end in slices:
          #~ newatoms.extend(self.data[start:end])
        #~ self.data = newatoms
        #~ self.unindex()
        #~ return True
    #~ 
    #~ 
    #~ def filter_residues_before_1(self):
        #~ """filter_residues_before_1() : deletes residues that have a negative or zero residue number
        #~ 
        #~ This is now run automatically in the Pdb constructor
        #~ """
        #~ slices = []
        #~ for start,end in self.xresidue_boundaries():
          #~ isok = self[start].ires >= 1
          #~ if isok:
            #~ if slices and slices[-1][1] == start:
              #~ slices[-1][1] = end
            #~ else:
    ...
    #~ 
    #~ 
    #~ def filter_duplicate_unlabelled_atoms(self):
        #~ """filter_duplicate_unlabelled_atoms() : deletes atoms that have the same residue and chain number, but different residue type. Keeps only the first residue.
          #~ 
          #~ This is now run automatically in the Pdb constructor
        #~ """
        #~ newatoms = []
        ...
        #~ for start,end in self.xresidue_boundaries():
          #~ for ix in xrange(start, end):
            #~ atm = self[ix]
            #~ atmhash = "%s %s %s %s %s"%(atm.ires, atm.atom, atm.chain, atm.altloc, atm.inscode)
            #~ if atmhash not in atmhashset:
              #~ newatoms.append(atm)
    ...
    #~ 
    #~ 
    #~ def filter_alternative_locations(self):
        #~ newatoms = []
        #~ 
        #~ for start,end in self.xresidue_boundaries():
          #~ atmset=[]
          #~ for ix in xrange(start, end):
            #~ atm = self[ix]
            #~ dsc=atm.atom
            #~ #if not atm.altloc or atm.altloc == "A":
            #~ if dsc not in atmset:
              #~ atmset.append(dsc)
              #~ newatoms.append(atm)
        #~ 
        #~ if len(newatoms) != len(self.data):
...
#~ 
    ...
    def filter(self, negres=True, noca=True, inscode=False, altloc=True, duperes=True):
        newatoms = []
        atmhashset = set([])
