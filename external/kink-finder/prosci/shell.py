...
class Params:
    ...
    def writeUsage(self, minimal=False):
        if None == self.usage:
          req = ""
          if self.required:
          ...
          sys.stderr.write(self.usage.replace("$0", self.scriptname))
          if self.usage[-1:] != "\n":
            sys.stderr.write("\n")
    ...
    def writeHelp(self):
        self.writeUsage(minimal=True)
        if self.help:
          sys.stderr.write(self.help.replace("$0", self.scriptname))
          if self.help[-1:] != "\n":
            sys.stderr.write("\n")
